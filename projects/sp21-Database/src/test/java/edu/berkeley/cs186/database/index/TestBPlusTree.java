...
public class TestBPlusTree {
    ...
    public void testMyBulkLoad() {
        ...
        Iterator<Pair<DataBox, RecordId>> pairIterator = pairs.iterator();
        float fillFactor = (float) 0.75;
        tree.bulkLoad(pairIterator, fillFactor);
        Iterator<RecordId> scan = tree.scanAll();
        int index = 0;
        while (scan.hasNext()) {
            assertEquals(scan.next(), rids.get(index));
            index++;
        }
    }
    ...
    public void testSimplePuts() {
        // This test aims to test put() in s simpler way than the test offered by berkeley.
        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        List<RecordId> sortedRids = new ArrayList<>();
        for (int i = 0; i < 100; ++i) {
        ...
        for (int d = 4; d < 5; ++d) {
            for (int n = 0; n < 2; ++n) {
                ...
                int count = 0;
                while (scan.hasNext()) {
                    assertEquals(scan.next(), new RecordId(count, (short) count));
                    count++;
                }
            }
        }
    ...
    public void testPDFTree() {
        ...
        try {
            ...
            String treePDFPath = currentFile.getCanonicalPath();
            System.out.println(treePDFPath);
            tree.toDotPDFFile(treePDFPath);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
