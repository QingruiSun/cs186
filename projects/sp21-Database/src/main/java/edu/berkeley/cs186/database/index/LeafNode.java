...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        Optional<RecordId> assertEmpty = getKey(key);
        if (assertEmpty.isPresent()) {
            throw new BPlusTreeException("Duplicate key when put in the LeafNode!");
        }
        int keySize = keys.size();
        // Put key in keys according to ascend order.
        int index = 0;
        while (index < keySize) {
        ...
        List<DataBox> secondKeys = new ArrayList<>();
        List<RecordId> secondRids = new ArrayList<>();
        for (int i = 0; i < order + 1; ++i) {
            secondKeys.add(keys.remove(order));
            secondRids.add(rids.remove(order));
        }
        LeafNode nextNode = new LeafNode(metadata, bufferManager, secondKeys, secondRids, rightSibling, treeContext);
        long newPageNum = nextNode.getPage().getPageNum();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        // Assume 0 < fillFactor <= 1.
        if ((fillFactor <= 0) || (fillFactor > 1)) {
            throw new RuntimeException("fillFactor is out of the illegal range!");
        }
        int order = metadata.getOrder();
        int legalSize = (int) (order * 2 * fillFactor);
        int index = keys.size();
        // Insert data item to LeafNode util the data is empty or the insert number is one more than the illegal size.
        while ((data.hasNext()) && (index <= legalSize)) {
            Pair<DataBox, RecordId> currentPair = data.next();
            keys.add(currentPair.getFirst());
            rids.add(currentPair.getSecond());
            index++;
        }
        if (keys.size() <= legalSize) {
            sync();
            return Optional.empty();
        }
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();
        // Move the last item in the overflowed node to the next new node.
        newKeys.add(keys.remove(legalSize));
        newRids.add(rids.remove(legalSize));
        ...
        newLeafNode.sync();
        sync();
        return Optional.of(new Pair<>(newKeys.get(0), newLeafNode.getPage().getPageNum()));
    }
