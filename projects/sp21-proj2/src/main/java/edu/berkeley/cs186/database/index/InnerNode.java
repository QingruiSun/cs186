...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        if (nextNode instanceof LeafNode) {
            ...
            InnerNode nextInnerNode = (InnerNode) nextNode;
            return nextInnerNode.get(key);
        }
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        InnerNode nextInnerNode = (InnerNode) nextNode;
        return nextInnerNode.getLeftmostLeaf();
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int index = InnerNode.numLessThanEqual(key, keys);
        BPlusNode nextNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(index));
        Optional<Pair<DataBox, Long>> newItem = nextNode.put(key, rid);
        if (!newItem.isPresent()) {
            return Optional.empty();
        }
        keys.add(index, newItem.get().getFirst());
        children.add(index + 1, newItem.get().getSecond());
        keySize = keySize + 1;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int order = metadata.getOrder();
        int keySize = keys.size();
        while ((data.hasNext()) && (keySize <= order * 2)) {
            BPlusNode rightmostNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(keySize));
            Optional<Pair<DataBox, Long>> newPair = rightmostNode.bulkLoad(data, fillFactor);
        ...
        InnerNode newNode = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);
        newNode.sync();
        DataBox popUpKey = keys.remove(order);
        sync();
        return Optional.of(new Pair<>(popUpKey, newNode.getPage().getPageNum()));
    }
