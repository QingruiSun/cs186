...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode possibleLeafNode = root.get(key);
        return possibleLeafNode.getKey(key);
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        Iterator<RecordId> resultIterator = new BPlusTreeIterator();
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> newItem = root.put(key, rid);
        if (!newItem.isPresent()) {
            return;
        }
        List<DataBox> newKeys = new ArrayList<>();
        newKeys.add(newItem.get().getFirst());
        List<Long> newChildren = new ArrayList<>();
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(newItem.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        while (data.hasNext()) {
            ...
            List<DataBox> newKeys = new ArrayList<>();
            newKeys.add(popUpPair.get().getFirst());
            List<Long> newChildren = new ArrayList<>();
            newChildren.add(root.getPage().getPageNum());
            newChildren.add(popUpPair.get().getSecond());
            InnerNode newRoot = new InnerNode(metadata, bufferManager, newKeys, newChildren, lockContext);
            updateRoot(newRoot);
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private LeafNode nowLeafNode;
        private Iterator<RecordId> nowIter;

        public BPlusTreeIterator() {
            LeafNode leftmostNode = root.getLeftmostLeaf();
            nowLeafNode = leftmostNode;
            nowIter = nowLeafNode.scanAll();
        }

        public BPlusTreeIterator(DataBox key) {
            nowLeafNode = root.get(key);
            List<DataBox> currentKeys = nowLeafNode.getKeys();
            int index = 0;
            int keySize = currentKeys.size();
            while (index < keySize) {
                if (currentKeys.get(index).compareTo(key) >= 0) {
                    break;
                }
                index++;
            }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if ((!nowIter.hasNext()) && (!nowLeafNode.getRightSibling().isPresent())) {
                return false;
            }
            while ((!nowIter.hasNext()) && (nowLeafNode != null)) {
                nowLeafNode = nowLeafNode.getRightSibling().get();
                nowIter = nowLeafNode.scanAll();
            }
            return nowIter.hasNext();
        }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            if (!hasNext()) {
                throw new NoSuchElementException("No remain RecordId in the BPlusTree!");
            }
            return nowIter.next();
        }
    }
